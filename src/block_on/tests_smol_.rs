...
//!
//! 该文件只在启用 `runtime-smol` feature 且处于测试构建（`cfg(test)`）时
//! 编译，不会影响正常的库构建。smol 的 `block_on`（底层为 `async_io::block_on`）
//! 会在当前线程上直接轮询 future 并驱动 async-io reactor，不依赖任何「环境
//! 运行时句柄」，因此这些测试与 tokio/compio 的测试在前提上有所不同。
