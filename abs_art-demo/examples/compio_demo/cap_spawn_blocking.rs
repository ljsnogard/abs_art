...
//! （compio 演示组，与 `examples/tokio_demo/cap_spawn_blocking.rs` 一一对应）：
//!
//! 1. **`spawn_blocking` 抽象**：把 CPU 密集 / 真正阻塞（IO、同步计算）的闭包
//!    投递到专用阻塞线程（compio 为每个阻塞任务起独立线程），避免占用
//!    运行时主线程——业务代码只依赖 `TrSpawnBlocking`，不感知后端线程池实现；
//! 2. **能力按需组合**：`BLOCK_ON | SPAWN_BLOCKING` 声明「阻塞等待 + 阻塞池」
//!    两种能力，既不缺也不多；
//! 3. 阻塞任务与异步任务**共存**：阻塞任务在后台线程跑，异步侧可以继续推进
//!    自己的逻辑。
//!
//! # 可以做到
//!
...
use bridge_compio::{BLOCK_ON, SPAWN_BLOCKING, Runtime, TrBlockOn, TrSpawnBlocking};
...
async fn blocking_plus_async() -> (usize, usize) {
    ...
    // 在 await 阻塞任务的同时，异步侧仍然可以继续做事——运行时主线程
...
fn main() {
    ...
    assert_eq!(count, 10, "异步侧计数不受阻塞线程影响");
    println!("compio cap_spawn_blocking OK: blocking_sum={sum}, async_count={count}");
}
